...
{
  if (regex_grouping_id_is_single_byte(id)) { return true; }
...
bool regex_ptree_definitionally_equal(ptree x, ptree y)
{
  ...
  bool r =
    regex_canonical_definitionally_equal(&strtab, xs, ys);
...
bool regex_canonical_is_atomic(stringtable_t *tab,
...
bool regex_canonical_atomic_and_equal(stringtable_t *tab,
                                      stringtable_index_t x,
...
bool regex_canonical_similar(stringtable_t *tab,
                             stringtable_index_t x,
                             stringtable_index_t y)
{
  ptree_context ctx = regex_ptree_create_context();
                                                ...
                                                intmap_t *map_arg)
{
  ...
  for (uint64_t size = 2; size = stack_size(&stack_libc, stack), size != 0;) {
...

    {
      ...
      if (regex_id_is_atomic(xid) &&
          regex_id_is_atomic(yid))
        {
          if (xid == yid) {continue; }
    ...
    if (intmap_available(map) < 4)
      {
        if (!intmap_rehash_double(&map)) { return false; }
        *map_arg = map;
      }
...
bool regex_canonical_equivalent(regex_cache_t *cache,
                                stringtable_index_t x,
                                stringtable_index_t y)
{
  ...
  bool res = regex_canonical_equivalent_with_structures(cache, x, y, &stack, &map);
  stack_destroy(&stack_libc, stack);
  intmap_destroy(map);
