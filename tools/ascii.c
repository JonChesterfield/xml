...
int main() {
  ...
  for (unsigned i = 0; i < 1024; i++) {
    int ok = (ptr_table[i] == str_table[i]);
    if (ok)
      continue;
    printf("[%u] %s %s %d ?= %d %c %c\n", i, ascii_char_to_name(i / 8),
           (ok ? "pass" : "fail"), (int)ptr_table[i], (int)str_table[i],
           ptr_table[i], str_table[i]);
  }

  for (unsigned i = 0; i < 256; i++) {
    char c = (char)i;
    assert(ascii_table_char_to_type(c) == ascii_stringtable_char_to_type(c));
