...
%.TokenList.prelint.xml:	%.TokenTree.xml common/TokenTree_to_TokenList.xsl | validate/TokenTree_to_TokenList
	@$(xmllint) --relaxng $(call get_schema_name, %common/TokenTree.rng) "$<" $(XMLLINTOPTS) --quiet
	@$(xsltproc) $(XSLTPROCOPTS) --output "$@" common/TokenTree_to_TokenList.xsl "$<"
...
%.TokenList.xml:	%.TokenList.pretty.xml | common/preconditions 
	@$(xmllint) --relaxng $(call get_schema_name, %common/TokenList.rng) "$<" $(XMLLINTOPTS) --quiet
	@cp "$<" "$@"
...
%.HexTokenList.prelint.xml:	%.TokenList.xml common/TokenList_to_HexTokenList.xsl | validate/TokenList_to_HexTokenList
	@$(xmllint) --relaxng $(call get_schema_name, %common/TokenList.rng) "$<" $(XMLLINTOPTS) --quiet
	@$(xsltproc) $(XSLTPROCOPTS) --output "$@" common/TokenList_to_HexTokenList.xsl "$<"
...
%.HexTokenList.xml:	%.HexTokenList.pretty.xml | common/preconditions 
	@$(xmllint) --relaxng $(call get_schema_name, %common/HexTokenList.rng) "$<" $(XMLLINTOPTS) --quiet
	@cp "$<" "$@"
...
%.RawBinary.prelint.xml:	%.HexTokenList.xml common/HexTokenList_to_RawBinary.xsl | validate/HexTokenList_to_RawBinary
	@$(xmllint) --relaxng $(call get_schema_name, %common/HexTokenList.rng) "$<" $(XMLLINTOPTS) --quiet
	@$(xsltproc) $(XSLTPROCOPTS) --output "$@" common/HexTokenList_to_RawBinary.xsl "$<"

%.RawBinary.pretty.xml:	%.RawBinary.prelint.xml | common/preconditions 
	@$(xsltproc) $(XSLTPROCOPTS) --output "$@" subtransforms/pretty.xsl "$<" || cp "$<" "$@"

%.RawBinary.xml:	%.RawBinary.pretty.xml | common/preconditions 
	@$(xmllint) --relaxng $(call get_schema_name, %common/RawBinary.rng) "$<" $(XMLLINTOPTS) --quiet
	@cp "$<" "$@"
